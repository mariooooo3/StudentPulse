    ...
    <motion.div className="p-5 sm:p-6 space-y-5" variants={staggerContainer} initial="hidden" animate="show">
            ...
            {studentRequests.slice(0, 3).map(request => (
              <motion.div key={request.id} whileHover={{ y: -1 }} className="flex items-center gap-3 rounded-xl bg-white/[0.03] border border-white/[0.05] px-4 py-3 hover:border-white/[0.09] transition-colors">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-600 truncate mt-0.5 font-mono">Gr. {request.group} Â· {request.room}</p>
                </div>
                <span className={clsx('rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide', STATUS_BADGE[request.status] ?? 'badge-amber')}>
                  {STATUS_LABEL[request.status] ?? 'ÃŽn aÈ™teptare'}
                </span>
              </motion.div>
            ))}
          </div>
      ...
      <motion.div variants={staggerItem} className="premium-card p-4 border-indigo-500/20 bg-indigo-500/[0.04]">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-400">
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-indigo-600/40 border border-indigo-400" />Ora ta curentÄƒ</div>
          ...
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-amber-900/40 border border-amber-700/50" />Aproape plin <span className="text-slate-600">(1â€“2 loc.)</span></div>
          ...
          <thead>
            ...
            <tr className="bg-white/[0.01]">
              ...
              {allCombos.map(combo => {
                ...
                return <th key={combo} className="px-3 py-2 text-[10px] font-semibold text-slate-600 border-b border-r border-white/[0.04] text-center whitespace-nowrap font-mono">{start}:00â€“{start + 2}:00</th>
              })}
            </tr>
          </thead>
          <tbody>
                  ...
                  return (
                    <td key={combo} className="px-2 py-2 border-r border-white/[0.04] text-center">
                      <button
                        ...
                        title={slot.isMine ? 'Ora ta curentÄƒ' : `${slot.room} Â· Gr. ${slot.group}`}
                      >
                        {availLabel(slot)}
                      </button>
                    </td>
      ...
      {pendingModal && (
        <RecoveryModal
          ...
          onConfirm={async (reason) => {
            ...
            onNotify?.({
              title: 'Cerere recuperare trimisÄƒ',
